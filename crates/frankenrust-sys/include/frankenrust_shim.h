/* The C half of `go_register_server_variables` (issue #11, mechanism from
 * #75's retracted trampoline design -- see shim.c for the full argument).
 *
...
typedef struct frankenrust_header_var {
   ...
   * NULL. */
  char *key;
  /* NULL with value_len 0 for an absent or empty value -- see this issue's
   * "empty-value rule" -- never a dangling non-NULL zero-length pointer. */
  char *value;
  size_t value_len;
...
/* Implemented in Rust:
 * crates/frankenrust-core/src/callbacks/servervars.rs.
 *
